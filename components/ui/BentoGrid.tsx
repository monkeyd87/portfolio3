...
export const BentoGridItem = ({
  ...
  const handleClick = ()=>{
    setCopied(true)
    navigator.clipboard.writeText('dennisdjack.dev@gmail.com')
  }
  return (
    <div
      className={cn(
    ...
    >
      <div className={`${id === 6 && "flex justify-center"} h-full`}>
        ...
        <div className={cn(
            titleClassName, 'transition duration-200 group-hover/bento:translate-x-2 relative md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-10'
        )}>
            ...
            {id===3 && (
              <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 ">
                <div className="flex flex-col gap-3 lg:gap-1">
                    {['Nextjs','React',"Node",'TypeScript'].map(item=>(
                ...
                <div className="flex flex-col gap-3 lg:gap-1">
                    <span className="py-4 px-3 text-center bg-[#10132e] rounded-lg"/>
                    {['MongoDB','express','Python'].map(item=>(
                      <span key={item} className="py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132e]">
                        {item}
                      </span>
                    ))}
            ...
            {id===6 && (
              <div className="mt-5 relative">
                <div className={`absolute -bottom-5 right-0`}>
                  ...
                  <div>
                    <MagicButton title={copied ? "Email Copied":"Copie Email"} position="left" otherClasses="!bg-[#161a31]" onClick={handleClick}/>
                  </div>
                </div>
